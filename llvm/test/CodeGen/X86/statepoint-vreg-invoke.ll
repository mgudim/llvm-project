...
define ptr addrspace(1) @test_invoke_same_val(i1 %cond, ptr addrspace(1) %val1, ptr addrspace(1) %val2, ptr addrspace(1) %val3)
  ...
  ; CHECK-NEXT:   renamable $r14 = COPY $rdx
  ; CHECK-NEXT:   renamable $ebp = COPY $edi
  ; CHECK-NEXT:   TEST8ri renamable $bpl, 1, implicit-def $eflags
  ; CHECK-NEXT:   JCC_1 %bb.3, 4, implicit killed $eflags
  ; CHECK-NEXT:   JMP_1 %bb.1
  ; CHECK-NEXT: {{  $}}
  ; CHECK-NEXT: bb.1.left:
  ; CHECK-NEXT:   successors: %bb.2(0x7ffff800), %bb.6(0x00000800)
  ; CHECK-NEXT:   liveins: $ebp, $rsi, $r14
  ; CHECK-NEXT: {{  $}}
  ...
  ; CHECK-NEXT:   liveins: $ebp, $rbx, $rsi, $r14
  ; CHECK-NEXT: {{  $}}
  ; CHECK-NEXT:   MOV64mr %stack.0, 1, $noreg, 0, $noreg, killed renamable $r14 :: (store (s64) into %stack.0)
  ; CHECK-NEXT:   EH_LABEL <mcsymbol >
  ; CHECK-NEXT:   ADJCALLSTACKDOWN64 0, 0, 0, implicit-def dead $rsp, implicit-def dead $eflags, implicit-def dead $ssp, implicit $rsp, implicit $ssp
  ; CHECK-NEXT:   $rdi = COPY killed renamable $rsi
  ...
  ; CHECK-NEXT:   successors: %bb.5(0x80000000)
  ; CHECK-NEXT:   liveins: $ebp, $rbx
  ; CHECK-NEXT: {{  $}}
  ; CHECK-NEXT:   renamable $r14 = MOV64rm %stack.0, 1, $noreg, 0, $noreg :: (load (s64) from %stack.0)
  ; CHECK-NEXT: {{  $}}
  ; CHECK-NEXT: bb.5.normal_return:
  ; CHECK-NEXT:   liveins: $ebp, $rbx, $r14
  ; CHECK-NEXT: {{  $}}
  ; CHECK-NEXT:   TEST8ri renamable $bpl, 1, implicit-def $eflags, implicit killed $ebp
  ; CHECK-NEXT:   renamable $rbx = CMOV64rr killed renamable $rbx, killed renamable $r14, 4, implicit killed $eflags
