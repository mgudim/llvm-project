...
; CHECK-NEXT:    ldrsb x0, [x8, x10]
; CHECK-NEXT:    tbz x0, #63, LBB0_3
; CHECK-NEXT:  LBB0_2: ; %cond.false.i.i
; CHECK-NEXT:    mov w1, #32768 ; =0x8000
; CHECK-NEXT:    ; kill: def $w0 killed $w0 killed $x0
; CHECK-NEXT:    stp x10, x8, [sp, #32] ; 16-byte Folded Spill
; CHECK-NEXT:    str x9, [sp, #16] ; 8-byte Spill
; CHECK-NEXT:    str w11, [sp, #12] ; 4-byte Spill
; CHECK-NEXT:    str w12, [sp, #28] ; 4-byte Spill
; CHECK-NEXT:    str x13, [sp] ; 8-byte Spill
; CHECK-NEXT:    bl ___maskrune
; CHECK-NEXT:  Lloh2:
; CHECK-NEXT:    adrp x14, __DefaultRuneLocale@GOTPAGE
; CHECK-NEXT:  Lloh3:
; CHECK-NEXT:    ldr x14, [x14, __DefaultRuneLocale@GOTPAGEOFF]
; CHECK-NEXT:    ldp x10, x8, [sp, #32] ; 16-byte Folded Reload
; CHECK-NEXT:    ldr x13, [sp] ; 8-byte Reload
; CHECK-NEXT:    ldr w12, [sp, #28] ; 4-byte Reload
; CHECK-NEXT:    ldr w11, [sp, #12] ; 4-byte Reload
; CHECK-NEXT:    ldr x9, [sp, #16] ; 8-byte Reload
; CHECK-NEXT:    cbz w0, LBB0_4
; CHECK-NEXT:    b LBB0_6
; CHECK-NEXT:  LBB0_3: ; %cond.true.i.i
; CHECK-NEXT:    add x15, x14, x0, lsl #2
; CHECK-NEXT:    ldr w15, [x15, #60]
; CHECK-NEXT:    and w0, w15, #0x8000
; CHECK-NEXT:    cbnz w0, LBB0_6
; CHECK-NEXT:  LBB0_4: ; %lor.rhs
; CHECK-NEXT:    ldrsb x0, [x9, x10]
; CHECK-NEXT:    tbnz x0, #63, LBB0_8
; CHECK-NEXT:  ; %bb.5: ; %cond.true.i.i217
; CHECK-NEXT:    add x15, x14, x0, lsl #2
; CHECK-NEXT:    ldr w15, [x15, #60]
; CHECK-NEXT:    and w0, w15, #0x8000
; CHECK-NEXT:    cbz w0, LBB0_9
; CHECK-NEXT:  LBB0_6: ; %while.body
; CHECK-NEXT:    ldrb w15, [x8, x10]
; CHECK-NEXT:    ldrb w16, [x9, x10]
...
; CHECK-NEXT:    orr w14, w12, w11
; CHECK-NEXT:    cbnz w14, LBB0_24
; CHECK-NEXT:  ; %bb.10: ; %if.then23
; CHECK-NEXT:    ldr x12, [x13, #16]
; CHECK-NEXT:    ldrb w11, [x8, x10]
...
; CHECK-NEXT:    add x12, x8, x10
; CHECK-NEXT:    add x8, x9, x10
; CHECK-NEXT:    add x9, x12, #1
...
; CHECK-NEXT:    ; =>This Inner Loop Header: Depth=1
; CHECK-NEXT:    add x13, x9, x11
; CHECK-NEXT:    ldrb w13, [x13, x10]
...
; CHECK-NEXT:    add x11, x8, x12
; CHECK-NEXT:    add x12, x12, #1
; CHECK-NEXT:    add x11, x11, x10
; CHECK-NEXT:    ldrb w11, [x11, #1]
