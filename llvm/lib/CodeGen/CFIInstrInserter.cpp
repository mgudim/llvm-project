...
void CFIInstrInserter::calculateCFAInfo(MachineFunction &MF) {
  ...
  for (MachineBasicBlock &MBB : MF) {
    ...
    MBBInfo.IncomingCSRLocations.resize(
      NumRegs,
      dwarf::UnwindLocation(dwarf::UnwindLocation::RegPlusOffset)
    );
...
void CFIInstrInserter::reportCSRError(const MBBCFAInfo &Pred,
  ...
  for (unsigned i = 0; i < Pred.OutgoingCSRLocations.size(); ++i) {
    errs() << "Dwarf register number: " << i << ", saved location: " << Pred.OutgoingCSRLocations[i] << "\n";
  }
  errs() << "\n";
  errs() << "Succ: " << Succ.MBB->getName() << " #" << Succ.MBB->getNumber()
         << " incoming CSR Saved: ";
  for (unsigned i = 0; i < Succ.IncomingCSRLocations.size(); ++i) {
    errs() << "Dwarf register number: " << i << ", saved location: " << Succ.OutgoingCSRLocations[i] << "\n";
  }
}
...
unsigned CFIInstrInserter::verify(MachineFunction &MF) {
  ...
  for (auto *CurrMBB : depth_first(&MF)) {
    ...
    for (MachineBasicBlock *Succ : CurrMBB->successors()) {
      ...
      if (SuccMBBInfo.IncomingCSRLocations != CurrMBBInfo.OutgoingCSRLocations) {
